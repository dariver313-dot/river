...
export function AdminTableLoading({ label, columns = 5 }: { label: string; columns?: number }) {
  return <div className="admin-table-loading" role="status" aria-label={label} style={{ "--admin-table-columns": columns } as CSSProperties}>
    ...
    <span className="admin-table-loading-caption"><LoaderCircle className="spin" size={16} aria-hidden="true" />正在读取…</span>
  </div>;
}
