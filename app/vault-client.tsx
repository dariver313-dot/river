...
function emptyCredentialForm(): CredentialForm {
  return { name: "", domain: "", username: "", password: "", category: "", group: "个人", twoFactor: false, totpEntries: [newTotpFormEntry()] };
}

function credentialFormHasTotp(form: CredentialForm) {
  return form.totpEntries.some((entry) => Boolean(entry.config) || Boolean(entry.value.trim()));
}
...
const securityIssueCopy: Record<SecurityIssue, { label: string; detail: string }> = {
  ...
  missing_two_factor: { label: "未开启双重验证", detail: "网站账户尚未标记启用二步验证" },
};
...
function VaultListLoading() {
  return <div className="vault-loading-rows" aria-busy="true">
    <LoadingIndicator label="正在同步项目" compact className="vault-loading-caption" />
...
function DetailPanelLoading() {
  return <aside className="detail-panel detail-panel-loading" aria-busy="true" aria-label="正在读取项目详情">
    <LoadingIndicator label="正在打开项目详情" compact className="detail-loading-caption" />
...
function AuditLogLoading() {
  return <div className="audit-log-loading" aria-busy="true">
    <LoadingIndicator label="正在读取操作审计" compact className="audit-loading-caption" />
...
function ProfileOverview({
  ...
  isLoading: boolean;
  securitySummaryReady: boolean;
  securityTotalItems: number;
...
function auditLabel(action: string) {
  const labels: Record<string, string> = {
    ...
    system_user_activation_resent: "重新发送了激活码",
    system_user_activation_started: "开始了账号激活",
    system_user_activated: "完成了账号激活",
    system_user_role_changed: "调整了系统用户角色",
    system_user_status_changed: "调整了系统用户状态",
    system_user_authenticator_reset: "重置了登录验证器",
    system_user_authenticator_recovery_started: "开始了验证器恢复",
    system_user_authenticator_recovered: "完成了验证器恢复",
    system_user_deleted: "删除了系统用户",
    profile_updated: "更新了个人资料",
    account_password_changed: "更新了登录密码",
    account_password_recovered: "通过安全邮箱恢复了登录密码",
    account_security_email_changed: "变更了安全邮箱",
    initial_admin_initialized: "完成了初始管理员配置",
    initial_admin_recovery_codes_rotated: "轮换了离线恢复码",
    initial_admin_recovered: "完成了初始管理员恢复",
...
function auditScopeLabel(action: string) {
  if (action.startsWith("system_user_") || action.startsWith("account_") || action.startsWith("initial_admin_") || action === "profile_updated") return "系统用户";
...
function embeddedAuditDetail(action: string, itemId: string | null) {
  ...
  try {
    const subject = JSON.parse(itemId) as { name?: unknown; origin?: unknown; path?: unknown; visibility?: unknown; enabled?: unknown };
    if (typeof subject.name !== "string" || typeof subject.origin !== "string") return null;
    const location = `${subject.origin}${typeof subject.path === "string" ? subject.path : ""}`;
    const details = [subject.name, location];
    if (subject.visibility === "admin") details.push("仅管理员");
    else if (subject.visibility === "all") details.push("全体用户");
    if (typeof subject.enabled === "boolean") details.push(subject.enabled ? "已启用" : "已停用");
    return details.join(" · ");
  } catch {
    return null;
  }
...
export default function VaultClient({ viewer: initialViewer }: { viewer: Viewer }) {
  ...
  // 只有密码库页会发起列表请求；直接进入其他页面时不应遗留初始加载态。
  ...
  const [securitySummaryReady, setSecuritySummaryReady] = useState(false);
  ...
  // 详情接口会返回解密后的凭据；不要因为列表加载或切换到其他页面而自动读取。
  const activeSelectedId = items.some((item) => item.id === selectedId) ? selectedId : null;
  ...
  const listTitle = collection === "security"
      ? securityFocus === "all" ? "待处理项目" : securityIssueCopy[securityFocus].label
  ...
  const addFormHasTotp = credentialFormHasTotp(form);
  const editFormHasTotp = credentialFormHasTotp(editForm);
  ...
  useEffect(() => {
    if (!serverSessionReady || (page !== "vault" && page !== "profile")) return;
    let cancelled = false;
    ...
    async function loadVault() {
      ...
      try {
        ...
        setSelectedId((current) => loadedItems.some((item) => item.id === current) ? current : null);
      } catch (error) {
        if (!cancelled) {
  ...
  }, [activeCategory, collection, debouncedVaultQuery, loadAttempt, page, securityFocus, serverSessionReady, sortOrder, space, vaultCurrentPage]);
  ...
  useEffect(() => {
    if (!serverSessionReady || page !== "vault" || !activeSelectedId) return;
    ...
    return () => { cancelled = true; controller.abort(); };
  }, [activeSelectedId, detailAttempt, page, requestVault, serverSessionReady]);

  useEffect(() => {
    if (page === "vault") return;
    // URL 后退/前进也必须释放已解密详情，不能只依赖侧边栏点击事件。
  ...
  async function saveCredential(credential: CredentialForm, userCode?: string) {
    ...
    try {
      const payload = await requestVault<{ item: VaultItem }>("/api/vault/items", {
        method: "POST",
        body: JSON.stringify({ ...credential, type: "登录", favorite: false, brand: "new", note: "", ...(userCode ? { userCode } : {}) }),
      });
      setVaultCurrentPage(1);
      setSelectedId(payload.item.id);
      setSelectedDetail(payload.item);
  ...
  function openEdit(item: VaultItem) {
    setEditForm({ name: item.name, domain: item.domain, username: item.username, password: item.password, category: item.category, group: item.group, twoFactor: item.twoFactor, totpEntries: item.totps.length > 0 ? item.totps.map((entry, index) => newTotpFormEntry(index, entry.config, entry.label)) : [newTotpFormEntry()] });
    setEditingItem(item);
    setPublicItemCode("");
    setRevealed(false);
    ...
    <div className="vault-app">
      ...
      <main ref={mainContentRef} id="main-content" className="main-shell" tabIndex={-1}>
        ...
        <section className="security-strip" aria-label="密码库凭据安全概览" aria-busy={isLoading}>
          ...
          </button> : <div className="risk-item risk-item-static"><span className="risk-icon risk-safe">{totalItems === 0 ? <ShieldCheck size={17} aria-hidden="true" /> : <Check size={17} aria-hidden="true" />}</span><span><strong id="security-heading">凭据安全评分 {totalItems === 0 ? "—/100" : `${securityScore}/100`}</strong><small>{totalItems === 0 ? "暂无数据" : "基础检查已通过"}</small></span></div>}
          {weakPasswordCount > 0 ? <button className="risk-item" onClick={() => openSecurityReview("weak_password")}><span className="risk-icon risk-danger"><AlertTriangle size={17} /></span><span><strong>{weakPasswordCount} 个密码长度不足</strong><small>少于 14 位</small></span><ChevronRight size={18} /></button> : <div className="risk-item risk-item-static"><span className="risk-icon risk-safe"><Check size={17} /></span><span><strong>{totalItems === 0 ? "暂无项目" : "密码长度正常"}</strong><small>{totalItems === 0 ? "等待数据" : "未发现短密码"}</small></span></div>}
          {reusedPasswordCount > 0 ? <button className="risk-item" onClick={() => openSecurityReview("reused_password")}><span className="risk-icon risk-warning"><ShieldEllipsis size={17} /></span><span><strong>{reusedPasswordCount} 个重复密码</strong><small>用于多个项目</small></span><ChevronRight size={18} /></button> : <div className="risk-item risk-item-static"><span className="risk-icon risk-safe"><Check size={17} /></span><span><strong>{totalItems === 0 ? "暂无项目" : "无重复密码"}</strong><small>{totalItems === 0 ? "等待数据" : "已保存密码不重复"}</small></span></div>}
          {missingTwoFactorCount > 0 ? <button className="risk-item" onClick={() => openSecurityReview("missing_two_factor")}><span className="risk-icon risk-info"><Smartphone size={17} /></span><span><strong>{missingTwoFactorCount} 个未启用双重验证</strong><small>按网站账户实际状态确认</small></span><ChevronRight size={18} /></button> : <div className="risk-item risk-item-static"><span className="risk-icon risk-safe"><Check size={17} /></span><span><strong>{totalItems === 0 ? "暂无项目" : "双重验证状态正常"}</strong><small>{totalItems === 0 ? "等待数据" : "全部项目已标记开启"}</small></span></div>}
        </section>
        ...
        <div className="content-grid">
          <section className="vault-panel" aria-labelledby="vault-list-title" aria-busy={isLoading}>
            {collection === "security" && <div className="security-review" role="region" aria-labelledby="security-review-title">
              <div className="security-review-head"><div><span className="eyebrow">密码库凭据检查</span><h2 id="security-review-title">{totalItems === 0 ? "暂无可检查项目" : securityIssueCount > 0 ? `${securityIssueCount} 条风险待处理` : "基础检查已通过"}</h2><p>{totalItems === 0 ? "暂无数据。" : "检查密码长度、重复使用和网站账户的二步验证状态。"}</p></div><button type="button" className="secondary-button" onClick={openAccountManagement}>全部项目</button></div>
            ...
            <div className="vault-list">
              {isLoading ? (
                ...
                <button key={item.id} className={`vault-row ${activeSelectedId === item.id ? "is-selected" : ""}`} onClick={() => { setSelectedId(item.id); setSelectedDetail(null); setDetailError(null); setRevealed(false); }} aria-pressed={activeSelectedId === item.id}>
                ...
                collection === "security" && totalItems > 0 && securityIssueCount === 0 ? <div className="empty-state empty-state-success"><ShieldCheck size={24} /><h3>基础检查已通过</h3><p>未发现基础风险。</p><button className="secondary-button" onClick={openAccountManagement}>全部项目</button></div> : <div className="empty-state"><Search size={24} /><h3>{totalItems === 0 ? "暂无数据" : collection === "security" ? "无匹配风险" : "无匹配项目"}</h3>{totalItems > 0 && <p>调整筛选条件后重试。</p>}{totalItems > 0 && <button className="secondary-button" onClick={clearVaultFilters}>清除筛选</button>}</div>
              )}
            </div>
          ...
          <aside className="detail-panel" aria-labelledby="detail-title">
            ...
            <aside className="detail-panel detail-empty" aria-live="polite">
              {items.length === 0 ? <><Search size={25} aria-hidden="true" /><h2>无匹配项目</h2><p>调整筛选条件后重试。</p><button className="secondary-button" onClick={clearVaultFilters}>清除筛选</button></> : detailError ? <><AlertTriangle size={25} aria-hidden="true" /><h2>无法读取项目详情</h2><p>{detailError}</p><button className="secondary-button" onClick={() => setDetailAttempt((current) => current + 1)}>重新读取</button></> : <><KeyRound size={25} aria-hidden="true" /><h2>选择一个项目</h2><p>点击左侧项目后查看登录信息、密码和验证器代码。</p></>}
            </aside>
          ) : (
        ...
        <div className="modal-layer" role="presentation">
          <section className="modal profile-editor-modal" role="dialog" aria-modal="true" aria-labelledby="profile-editor-title">
            ...
            <form className="modal-form" onSubmit={saveProfile}>
              ...
              <footer className="modal-footer"><button type="button" className="secondary-button" onClick={() => setShowProfileEditor(false)} disabled={isSaving}>取消</button><button type="submit" className="primary-button" disabled={isSaving}>{isSaving ? <LoadingMark className="button-loading-mark" /> : <Check size={17} />}{isSaving ? "正在保存" : "保存资料"}</button></footer>
            </form>
          </section>
        </div>
      )}
        ...
        <div className="modal-layer" role="presentation">
          <section className="modal credential-modal" role="dialog" aria-modal="true" aria-labelledby="add-title">
            ...
            <form className="modal-form" onSubmit={submitCredential}>
              ...
              <label className="credential-two-factor-toggle"><input type="checkbox" checked={form.twoFactor || addFormHasTotp} disabled={addFormHasTotp} onChange={(event) => setForm({ ...form, twoFactor: event.target.checked })} /><span><strong>该网站账户已启用双重验证</strong><small>{addFormHasTotp ? "已填写验证器密钥，因此会自动记录为已启用。" : "不保存验证器密钥时，也可以单独记录实际启用状态。"}</small></span></label>
              ...
              <footer className="modal-footer"><button type="button" className="secondary-button" onClick={() => setShowAdd(false)} disabled={isSaving}>取消</button><button type="submit" className="primary-button" disabled={isSaving}>{isSaving ? <LoadingMark className="button-loading-mark" /> : <Plus size={17} />}{isSaving ? "正在保存" : "添加项目"}</button></footer>
            </form>
          </section>
        </div>
      )}
        ...
        <div className="modal-layer" role="presentation">
          <section className="modal credential-modal" role="dialog" aria-modal="true" aria-labelledby="edit-title">
            ...
            <form className="modal-form" onSubmit={submitEditCredential}>
              ...
              <label className="credential-two-factor-toggle"><input type="checkbox" checked={editForm.twoFactor || editFormHasTotp} disabled={editFormHasTotp} onChange={(event) => setEditForm({ ...editForm, twoFactor: event.target.checked })} /><span><strong>该网站账户已启用双重验证</strong><small>{editFormHasTotp ? "已保存或填写验证器密钥；移除密钥后可单独关闭此状态。" : "此状态与是否在密码库保存验证器密钥相互独立。"}</small></span></label>
              ...
              <footer className="modal-footer"><button type="button" className="secondary-button" onClick={() => { setPublicItemCode(""); setEditingItem(null); }} disabled={isSaving}>取消</button><button type="submit" className="primary-button" disabled={isSaving}>{isSaving ? <LoadingMark className="button-loading-mark" /> : <Check size={17} />}{isSaving ? "正在保存" : "保存变更"}</button></footer>
            </form>
          </section>
        </div>
      )}
        ...
        <div className="modal-layer" role="presentation">
          <section className="modal user-create-modal" role="dialog" aria-modal="true" aria-labelledby="publish-public-title">
            ...
            <form className="modal-form" onSubmit={(event) => { event.preventDefault(); if (!/^\d{6}$/.test(publicItemCode)) { setToast("请输入 6 位 Google 验证码。"); return; } if (pendingPublicPublish.item) void saveEditedCredential(pendingPublicPublish.item, pendingPublicPublish.form, publicItemCode); else void saveCredential(pendingPublicPublish.form, publicItemCode); }}>
              ...
              <footer className="modal-footer"><button type="button" className="secondary-button" onClick={() => { setPublicItemCode(""); setPendingPublicPublish(null); }} disabled={isSaving}>返回编辑</button><button type="submit" className="primary-button" disabled={isSaving}>{isSaving ? <LoadingMark className="button-loading-mark" /> : <UsersRound size={17} />}{isSaving ? "正在发布" : "确认发布"}</button></footer>
            </form>
          </section>
        </div>
      )}
        ...
        <div className="modal-layer" role="presentation">
          <section className="modal danger-modal" role="dialog" aria-modal="true" aria-labelledby="delete-title">
            ...
            <form className="modal-form" onSubmit={(event) => { event.preventDefault(); deleteCredential(); }}>
              ...
              <footer className="modal-footer"><button type="button" className="secondary-button" onClick={() => { setDeleteItemCode(""); setDeleteTarget(null); }} disabled={isSaving}>取消</button><button type="submit" className="danger-button" disabled={isSaving}>{isSaving ? <LoadingMark className="button-loading-mark" /> : <Trash2 size={17} />}{isSaving ? "正在删除" : "删除项目"}</button></footer>
            </form>
          </section>
        </div>
      )}
        ...
        <div className="modal-layer" role="presentation">
          <section className="modal user-create-modal" role="dialog" aria-modal="true" aria-labelledby="create-system-user-title">
            ...
            <form className="modal-form" onSubmit={createSystemUser}>
              ...
              <footer className="modal-footer"><button type="button" className="secondary-button" onClick={() => { setSystemUserCode(""); setShowUserCreateDialog(false); }} disabled={isSaving}>取消</button><button type="submit" className="primary-button" disabled={isSaving}>{isSaving ? <LoadingMark className="button-loading-mark" /> : <UserCog size={17} />}{isSaving ? "正在创建" : "创建用户"}</button></footer>
            </form>
          </section>
        </div>
      )}
