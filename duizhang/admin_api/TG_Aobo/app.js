...
const CFG = {
  ...
  PORT: safeParseInt(process.env.PORT, 3020),
  ...
  FIXED_UA: "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36",
...
const DB = {
  ...
  insertTxLog(items, todayMap) {
    ...
    return this._safe(() => {
      ...
      const existing = todayMap || (uniquePairs.length > 0 ? this.getTodayBatch(uniquePairs) : new Map());
    ...
    try {
      ...
      if (typeof res.data === "string") {
        ...
        if (trimmed.startsWith("<")) {
          ...
          log.error("API返回HTML而非JSON", {
            status: res.status,
            url: String(url).slice(0, 100),
            contentType: respHeaders["content-type"] || respHeaders["Content-Type"] || "",
            server: respHeaders["server"] || respHeaders["Server"] || "",
            cfRay: respHeaders["cf-ray"] || respHeaders["Cf-Ray"] || "",
            xPoweredBy: respHeaders["x-powered-by"] || respHeaders["X-Powered-By"] || "",
            contentLength: respHeaders["content-length"] || respHeaders["Content-Length"] || "",
...
function generateGoogleCode() {
  try {
    ...
    log.error("谷歌验证码生成失败", { err: e?.message || String(e) });
    return null;
  }
}

// ================================================================
//  7b. TOKEN / API — Cookie/Bearer System
// ================================================================
function generateHeaders(token) {
  ...
  return {
    ...
    Cookie: `language=zh-CN; GooglekeY=1; enable2fa=true; token=${bearer || ""}; sidebarStatus=0`,
    Origin: CFG.BASE_URL,
...
function parseToken(raw) {
  ...
  const jwt = extractBearerToken(trimmed);
  if (!jwt || jwt.split(".").length !== 3) throw new Error("JWT格式无效");
  return trimmed;
}
...
async function checkUserExists(members, token) {
  ...
  const tasks = uniqueMembers.map((account) => (async () => {
    try {
      const url = `${CFG.BASE_URL}/api/admin/member/getAccountDetail?account=${encodeURIComponent(account)}&popularizeId=&currency=CNY`;
      const res = await httpRequest("GET", url, { headers: generateHeaders(token) });
      ...
      const data = res.data;
      if (data.code === 401 || data.status === 401 || (data.msg && data.msg.includes("未登录"))) {
        tokenExpired = true;
        resultMap.set(account, { exists: false });
        return;
      }
...
// ================================================================
//  7c. API — 加款接口 (rechargeOrder/manual)
// ================================================================
async function apiAddBalance(member, remark, amount, token, cachedInfo) {
  ...
  try {
    ...
    if (cachedInfo && cachedInfo.nickname && cachedInfo.id) {
      ...
      const url = `${CFG.BASE_URL}/api/admin/member/getAccountDetail?account=${encodeURIComponent(member.trim())}&popularizeId=&currency=CNY`;
      const userRes = await httpRequest("GET", url, { headers: generateHeaders(token) });

      if (!userRes.data || typeof userRes.data !== "object") {
        log.error("getAccountDetail返回异常", { member, dataType: typeof userRes.data, dataPreview: safeStringify(userRes.data).slice(0, 300) });
        return { ok: false, err: "查询用户信息失败" };
      }

      const userData = userRes.data;
      if (userData.code === 401 || userData.status === 401) {
        log.error("getAccountDetail返回401", { member, code: userData.code, status: userData.status, msg: userData.msg || userData.message || "" });
        return { ok: false, err: "Token已过期, 请重新设置Token" };
      }
      if (userData.data == null) return { ok: false, err: "用户不存在" };

      nickname = userData.data.nickname || member.trim();
      memberId = userData.data.id;
      if (!memberId) return { ok: false, err: "用户ID缺失, 无法加款" };
    }

    const params = new URLSearchParams({
      nickname, memberId, remarks: remark,
      discountDmlMultiple: "1", discount: "0", discountAmount: String(amount),
      dmlFlag: "1", discountType: "888", discountDml: String(amount),
      skipAuditing: "true", amount: "0", normalDm: "0", pointFlag: "1", currency: "CNY",
      gs: googleCode,
    });

    const addUrl = `${CFG.BASE_URL}/api/admin/finance/rechargeOrder/manual?${params.toString()}`;
    const res = await httpRequest("POST", addUrl, {
      headers: generateHeaders(token),
    });

    if (!res.data || typeof res.data !== "object") {
      log.error("rechargeOrder返回异常", { member, dataType: typeof res.data, dataPreview: safeStringify(res.data).slice(0, 300) });
      return { ok: false, err: "API返回格式异常" };
    }

    const data = res.data;
    if (data.code === 401 || data.status === 401) {
      log.error("rechargeOrder返回401", { member, code: data.code, status: data.status, msg: data.msg || data.message || "" });
      return { ok: false, err: "Token已过期, 请重新设置Token" };
    }

    if (data.succeed === true || data.succeed === "true") return { ok: true };

    const apiMsg = data.message || data.msg || data.error || "";
...
//  8. PARSER
// ================================================================
const TOKEN_RE = /^\s*(?:Bearer\s+)?eyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\s*$/;
...
function buildConfirmAGroupNotification(results) {
  ...
  const p = [];
  if (ok.length) { p.push(`✅ 已加: ${ok.length}笔`); ok.forEach((r) => p.push(`  ${r.member} ${r.remark} ${r.amount}`)); }
...
async function handleB(ctx, entries) {
  try {
    ...
    let notExistMembers = [];
    if (GLOBAL_TOKEN && circuit.isAvailable()) {
      const { map: existMap, tokenExpired, networkError } = await checkUserExists(entries.map(e => e.member), GLOBAL_TOKEN);
...
bot.action(/^ok_(.+)$/, async (ctx) => {
  try {
    ...
    const okList = (results || []).filter((r) => r && r.ok);
    if (okList.length) DB.insertTxLog(okList, cached.successMap);
