...
const totp = (() => {
  try {
    const secret = process.env.GOOGLE_TOTP_SECRET;
    if (!secret || typeof secret !== "string" || !secret.trim()) {
      log.warn("GOOGLE_TOTP_SECRET 未设置, 加款接口将无法使用谷歌验证码");
      return null;
    }
    return new TOTP({
      issuer: "LIVE",
    ...
    log.error("TOTP初始化失败", { err: e?.message || String(e) });
    return null;
  }
})();

function generateGoogleCode() {
  if (!totp) {
    log.error("generateGoogleCode: TOTP未初始化");
    return null;
  }
  try {
    return totp.generate();
  } catch (e) {
    log.error("generateGoogleCode失败", { err: e?.message || String(e) });
    return null;
  }
}

log.info("配置加载完成", { admin: CFG.ADMIN_ID, gA: CFG.GROUP_A_ID, gB: CFG.GROUP_B_ID, port: CFG.PORT, totp: !!totp });

// ================================================================
//  3. HTTP AGENTS
// ================================================================
const apiHttpAgent = new http.Agent({ keepAlive: true, maxSockets: 10, timeout: CFG.API_CONNECT_TIMEOUT });
const apiHttpsAgent = new https.Agent({ keepAlive: true, maxSockets: 10, timeout: CFG.API_CONNECT_TIMEOUT });
...
function initDB() {
  try {
    ...
    db.exec("DROP TABLE IF EXISTS transaction_logs");
    ...
    return this._safe(() => {
      ...
      for (const e of entries) {
        if (e && e.member && e.remark) {
          ...
          if (!seen.has(key)) { seen.add(key); pairs.push(e); }
        }
      }
...
const circuit = {
  ...
  recordSuccess() {
    if (this.state === "open" || this.state === "half-open") { this.state = "closed"; log.info("熔断器恢复", { state: "closed" }); }
    this.failures = 0;
  },

  recordFailure() {
    if (this.state === "half-open") { this.state = "open"; this._openAt = Date.now(); return; }
    this.failures++;
    if (this.failures >= CFG.CIRCUIT_FAILURES) { this.state = "open"; this._openAt = Date.now(); }
  },

  isAvailable() {
    if (this.state === "closed") return true;
    if (this.state === "open") {
      if (Date.now() - this._openAt >= CFG.CIRCUIT_COOLDOWN) { this.state = "half-open"; return true; }
      return false;
    }
    return true;
  },
...
async function httpRequest(method, url, options = {}, retries = CFG.RETRY) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      const res = await axios({ method, url, ...merged });
      ...
      if (res.status === 429) {
        ...
        if (attempt === retries) { circuit.recordFailure(); break; }
        await sleep(waitMs);
        continue;
      }
      ...
      if (res.data === null || res.data === undefined) throw new Error("API返回空响应");
      if (typeof res.data === "string") {
        ...
        const requestToken = options?._sm4Token;
        if (requestToken) {
          const decrypted = sm4Decrypt(trimmed, requestToken);
          if (decrypted) {
            try { res.data = JSON.parse(decrypted); } catch {
              const lastBrace = decrypted.lastIndexOf("}");
              if (lastBrace >= 0) {
                try { res.data = JSON.parse(decrypted.substring(0, lastBrace + 1)); } catch {
                  log.warn("SM4解密后JSON解析失败", { url: String(url).slice(0, 80) });
                  throw new Error("SM4解密后JSON解析失败, 请检查Token是否正确");
                }
              } else {
                log.warn("SM4解密后JSON解析失败", { url: String(url).slice(0, 80) });
                throw new Error("SM4解密后JSON解析失败, 请检查Token是否正确");
            ...
            try { res.data = JSON.parse(trimmed); } catch { throw new Error("SM4解密失败, 请检查Token是否正确"); }
          }
        } else {
          try { res.data = JSON.parse(trimmed); } catch { throw new Error("API返回无效JSON"); }
        }
      }
...
// ================================================================
//  7. TOKEN / API — TIK System X-AUTH-TOKEN
// ================================================================
let _sm4KeyCache = { token: null, key: null };

function deriveSM4Key(token) {
  if (_sm4KeyCache.token === token && _sm4KeyCache.key) return _sm4KeyCache.key;
  const key = crypto.createHash("md5").update(token).digest("hex").toUpperCase();
  _sm4KeyCache = { token, key };
  return key;
}

function sm4Decrypt(encryptedB64, token) {
  try {
    const key = deriveSM4Key(token);
    const decrypted = SM4.decrypt(encryptedB64, key, {
    ...
    log.warn("SM4解密失败", { err: e?.message });
    return null;
  }
}
...
function parseToken(raw) {
  ...
  if (trimmed.length > 10000) throw new Error("Token过长");
  return trimmed;
}

function validateToken(token) {
  if (!token || typeof token !== "string" || !token.trim()) return { ok: false, err: "Token未设置或为空" };
  try { parseToken(token); return { ok: true }; }
  catch (e) { return { ok: false, err: "Token格式无效: " + (e?.message || "未知错误") }; }
}

// ================================================================
//  7b. API — 用户查询 (memberAccount/list)
// ================================================================
async function checkUserExists(members, token) {
  ...
  let parsedToken;
  try { parsedToken = parseToken(token); } catch { return { map: new Map(), tokenExpired: false, networkError: false }; }
  ...
  const tasks = uniqueMembers.map((account) => (async () => {
    try {
      const url = `${CFG.BASE_URL}/livepro/memberAccount/list?currentPage=1&pageSize=50&memberName=${encodeURIComponent(account)}`;
      const res = await httpRequest("GET", url, { headers: generateHeaders(parsedToken), _sm4Token: parsedToken });
      ...
      const data = res.data;
      if (data.code === "401" || (data.success === false && data.msg && data.msg.includes("未登录"))) {
        tokenExpired = true;
        resultMap.set(account, { exists: false });
        return;
      }
      ...
      const match = items.find(item => item && item.memberName === account);
      if (match) {
        resultMap.set(account, { exists: true, nickname: match.memberName || account, id: match.accountId || match.memberId || null });
      } else {
        resultMap.set(account, { exists: false });
      }
    } catch (e) {
...
async function apiAddBalance(member, remark, amount, token, cachedInfo) {
  ...
  let parsedToken;
  try { parsedToken = parseToken(token); } catch { return { ok: false, err: "Token解析失败" }; }

  const account = member.trim();
  if (!account) return { ok: false, err: "用户名为空" };

  try {
    let nickname, memberId;
    if (cachedInfo && cachedInfo.nickname && cachedInfo.id) {
      nickname = cachedInfo.nickname;
      memberId = cachedInfo.id;
    } else {
      const url = `${CFG.BASE_URL}/livepro/memberAccount/list?currentPage=1&pageSize=50&memberName=${encodeURIComponent(account)}`;
      const userRes = await httpRequest("GET", url, { headers: generateHeaders(parsedToken), _sm4Token: parsedToken });

      if (!userRes.data || typeof userRes.data !== "object") return { ok: false, err: "查询用户信息失败" };

      const userData = userRes.data;
      if (userData.code === "401" || (userData.success === false && userData.msg && userData.msg.includes("未登录"))) return { ok: false, err: "Token已过期, 请重新设置Token" };

      const items = Array.isArray(userData.items) ? userData.items : [];
      const match = items.find(item => item && item.memberName === account);
      if (!match) return { ok: false, err: "用户不存在" };

      nickname = match.memberName || account;
      memberId = match.accountId || match.memberId || null;
    }

    if (!memberId) return { ok: false, err: "用户ID缺失, 无法加款" };

    const googleCode = generateGoogleCode();
    if (!googleCode) return { ok: false, err: "谷歌验证码生成失败, 请检查GOOGLE_TOTP_SECRET配置" };

    const url = `${CFG.BASE_URL}/livepro/memberAccount/updateBalance`;
    const requestData = {
      totalNum: 1,
      transDetail: "礼金",
      transType: 174,
      ...
      list: [{ memberName: nickname || account, amount: amount }],
    };

    const res = await httpRequest("POST", url, {
      headers: generateHeaders(parsedToken),
      data: requestData,
      _sm4Token: parsedToken,
    });
    ...
    if (data.success === true || data.success === "true" || data.code === "200") {
      const succNum = data.succNum !== undefined ? data.succNum : data.data?.succNum;
      const transResult = data.transResult !== undefined ? data.transResult : data.data?.transResult;
      if ((Number(succNum) > 0 && Number(transResult) === 0) || data.transResultDesc === "成功") {
        return { ok: true };
      }
    }

    const apiMsg = data.msg || data.transResultDesc || data.message || data.error || "";
...
// ================================================================
//  7d. API — 按真实姓名查询用户 (getAllUsersByCondition)
// ================================================================
async function apiCheckOldUsers(realname, token) {
  ...
  try {
    const url = `${CFG.BASE_URL}/livepro/userCenter/getAllUsersByCondition`;
    const requestData = {
      currentPage: 1,
      key: 2,
      pageSize: 50,
    ...
    if (data.success === true || data.code === "200") {
      const items = Array.isArray(data.items) ? data.items : [];
      return { ok: true, items, totalNum: data.totalNum || items.length };
    }

    const apiMsg = data.msg || data.message || data.error || "";
    return { ok: false, err: apiMsg ? String(apiMsg) : "查询失败" };
...
//  8. PARSER
// ================================================================
const TOKEN_RE = /^\s*[A-Za-z0-9+/!=]{10,}={0,2}(\r?\n\s*[A-Za-z0-9+/!=]{10,}={0,2})?\s*$/;
    ...
    const p = (async () => {
      ...
      try {
        const r = await Promise.race([
          apiAddBalance(item.member, item.remark, item.amount, token, existMap?.get(item.member)),
...
function buildConfirmAGroupNotification(results) {
  ...
  if (fail.length) p.push(`❌ 加款失败：${fail.length} 笔`);
  return p.join("\n");
}
...
bot.command("checkoldusers", async (ctx) => {
  try {
    ...
    for (const u of items) {
      const profitVal = parseFloat(u.profitAndLoss) || 0;
      const profitText = profitVal < 0 ? `输钱：${Math.abs(profitVal)}` : (profitVal > 0 ? `赢钱：${profitVal}` : "赢钱：0");
      const statusText = u.freezeStatus === 0 ? "正常" : "禁用";
...
bot.on("text", async (ctx) => {
  try {
    ...
    if (TOKEN_RE.test(rawText) && !LINE_RE.test(rawText)) {
      const tokenCheck = validateToken(rawText);
      if (tokenCheck.ok) {
        GLOBAL_TOKEN = rawText.trim();
        _sm4KeyCache = { token: null, key: null };
        ...
        await tg.reply(ctx, `❌ ${tokenCheck.err}`);
      }
      return;
    }
...
async function handleB(ctx, entries) {
  try {
    ...
    if (GLOBAL_TOKEN && circuit.isAvailable()) {
      const { map: eMap, tokenExpired, networkError } = await checkUserExists(entries.map(e => e.member), GLOBAL_TOKEN);
      existMap = eMap;
...
bot.action(/^no_(.+)$/, async (ctx) => {
  try {
    ...
    const isExpired = cached && Date.now() > cached.expire;
    ...
    } else if (cached && isExpired && cached.sourceChatId) {
      let aText = "";
      if (cached.overLimit && cached.overLimit.length) {
        aText += `⛔️ 拒绝加款：${cached.overLimit.length} 笔`;
        cached.overLimit.forEach(r => { aText += `\n${r.member}  ${r.remark}  ${r.amount} - 金额超限`; });
    ...
    const header = cached ? (cached.header || null) : null;
    const baseReport = cached ? buildBReport({ overLimit: cached.overLimit || [] }, cached.autoResults, cached.notExist || []) : null;
