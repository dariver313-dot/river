...
function loadConfig() {
    ...
    if (!fs.existsSync(configPath)) {
        console.warn('[qyt] config.json not found, using empty config');
        configJson = {};
        return;
    }
    try {
        configJson = JSON.parse(fs.readFileSync(configPath, 'utf8'));
        console.log('[qyt] config.json loaded');
    } catch (e) {
        console.error('[qyt] failed to parse config.json:', e.message);
        configJson = {};
    }
}
...
function buildShellHtml() {
    ...
    return {
        html: [
            ...
            '<title>Loading...</title>',
            '<style>',
            '*{margin:0;padding:0;box-sizing:border-box}html,body{height:100%;background:#1e40af}',
            '.loader-wrap{display:flex;flex-direction:column;align-items:center;justify-content:center;position:fixed;left:0;top:0;width:100vw;height:100vh;z-index:999999;background:#1e40af;font-family:sans-serif;transition:opacity 200ms ease-out}',
            '.loader-spinner{width:36px;height:36px;border:3px solid rgba(255,255,255,0.15);border-top-color:#5ab0ff;border-radius:50%;animation:spin .8s linear infinite}',
...
fs.watch(path.join(ROOT, 'page.html'), function (event) {
    if (event === 'change') {
        try {
            ...
            invalidateCache();
            console.log('[qyt] page.html changed, cache invalidated');
        } catch (e) {
            console.error('[qyt] failed to reload page.html:', e.message);
        }
    }
});
...
fs.watch(path.join(ROOT, 'config.json'), function (event) {
    if (event === 'change') {
        try {
            loadConfig();
            invalidateCache();
            console.log('[qyt] config.json changed, cache invalidated');
        } catch (e) {
            console.error('[qyt] failed to reload config.json:', e.message);
        }
    }
});
...
app.post('/key', function (req, res) {
    ...
    try {
        ...
        console.error('[qyt] RSA encrypt failed:', e.message);
...
var server = app.listen(PORT, function () {
    console.log('[qyt] v1.0.0 running on http://localhost:' + PORT);
});

function gracefulShutdown() { console.log('[qyt] Shutting down...'); server.close(function () { process.exit(0); }); setTimeout(function () { process.exit(1); }, 5000); }
