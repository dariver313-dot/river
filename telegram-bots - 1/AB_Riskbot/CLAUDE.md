...
4. **Notification** (`telegram.ts`) — Sends risk alerts to a Telegram group via grammy bot. High/CRITICAL/MEDIUM orders get inline keyboards for manual review, hedge-check queries, and association queries. Orders with betting violations (R24-R26) that remain unreviewed for 3 minutes get auto-reviewed.
