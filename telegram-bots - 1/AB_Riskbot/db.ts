...
async function verifySchemaConsistency(): Promise<void> {
  const expectedColumns: Record<string, string[]> = {
    RiskEval: ['orderId', 'memberId', 'memberName', 'totalScore', 'riskLevel', 'triggeredRules', 'detail', 'notified', 'finalStatus', 'feedback', 'createdAt'],
    RuleFeedback: ['id', 'evalId', 'ruleId', 'feedback', 'memberId', 'periodInfo', 'createdAt'],
    BotConfig: ['id', 'key', 'value'],
    MemberProfile: ['memberName', 'memberId', 'evalCount', 'highRiskCount', 'maxRiskLevel', 'lastEvalAt', 'lastEvalScore', 'topRules', 'totalWithdrawAmount', 'recentWithdrawTrend', 'updatedAt'],
  ...
  try {
    ...
    for (const [tableName, requiredCols] of Object.entries(expectedColumns)) {
      ...
      if (missing.length > 0) {
        ...
        if (tableName === 'RiskEval' || tableName === 'BotConfig') {
          criticalIssues.push(msg);
        } else {
          logger.warn({ table: tableName, missing }, `[数据库] ${msg}，可能需要手动执行 prisma db push`);
        }
      }
    }
...
export async function ensureDatabase(): Promise<void> {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      execFile(process.execPath, [prismaCli, 'db', 'push', '--accept-data-loss'], {
    ...
    try {
      ...
      const ddl: readonly string[] = [
        ...
        `CREATE TABLE IF NOT EXISTS RiskEval (id TEXT PRIMARY KEY, orderId TEXT NOT NULL UNIQUE, memberId TEXT NOT NULL, memberName TEXT NOT NULL DEFAULT '', totalScore INTEGER NOT NULL DEFAULT 0, riskLevel TEXT NOT NULL DEFAULT 'LOW', triggeredRules TEXT NOT NULL DEFAULT '[]', detail TEXT NOT NULL DEFAULT '{}', notified INTEGER NOT NULL DEFAULT 0, finalStatus TEXT NOT NULL DEFAULT '', feedback TEXT NOT NULL DEFAULT '', createdAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
        ...
        `CREATE INDEX IF NOT EXISTS RuleFeedback_evalId_ruleId_idx ON RuleFeedback(evalId, ruleId)`,
