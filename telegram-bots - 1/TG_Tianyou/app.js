...
const CFG = {
  ...
  // ─── 固定 User-Agent ───
  FIXED_UA: "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.0 Safari/537.36",
...
const DB = {
  ...
  getTodayMemberCounts(members) {
    if (!this._isAlive() || !Array.isArray(members) || members.length === 0) return new Map();
    return this._safe(() => {
      const unique = [...new Set(members.map(m => String(m).trim()).filter(Boolean))];
      if (unique.length === 0) return new Map();
      const placeholders = unique.map(() => "?").join(",");
      const sql = `SELECT member, COUNT(*) as cnt FROM tx_log WHERE member IN (${placeholders}) AND date(created_at) = date('now','+8 hours') GROUP BY member`;
      const rows = db.prepare(sql).all(...unique);
      const map = new Map();
      for (const r of (rows || [])) {
        map.set(String(r.member).trim(), r.cnt);
      }
      return map;
    }) || new Map();
  },
...
function classifyEntries(entries) {
  ...
  const members = [...new Set(deduped.map(e => String(e.member).trim()))];
  const memberCounts = members.length > 0 ? DB.getTodayMemberCounts(members) : new Map();
  ...
  for (const e of deduped) {
    try {
      ...
      const todayCount = memberCounts.get(String(e.member).trim()) || 0;
      if (todayCount >= 3) {
        confirm.push({ ...e, _freqWarn: true, _todayCount: todayCount });
        continue;
      }
...
function buildAGroupNotification(classified, results, notExist) {
  ...
  if (classified.confirm && classified.confirm.length) {
    ...
    classified.confirm.forEach((r) => {
      let tag = "";
      if (r._freqWarn) tag = ` [今日赠送${(r._todayCount || 0) + 1}次]`;
      p.push(`${r.member}  ${r.remark}  ${r.amount}${tag}`);
    });
  }
  const ok = (results || []).filter(r => r && r.ok);
  ...
  try {
    ...
    if (classified.confirm.length) {
      ...
      for (const t of classified.confirm) {
        let tag = "";
        if (t._freqWarn) tag = ` [今日赠送${(t._todayCount || 0) + 1}次]`;
        bText += `\n${t.member || "?"}  ${t.remark || "?"}  ${t.amount}${tag}`;
      }
...
bot.action(/^no_(.+)$/, async (ctx) => {
  try {
    ...
    const header = cached.header || null;
    const baseReport = buildBReport({ overLimit: cached.overLimit || [] }, cached.autoResults, cached.notExist || []);
    if (cached && cached.sourceChatId && cached.aGroupMsgId) {
      let aText = "";
      if (cached.overLimit && cached.overLimit.length) {
        aText += `⛔️ 拒绝加款：${cached.overLimit.length} 笔`;
        cached.overLimit.forEach(r => { aText += `\n${r.member}  ${r.remark}  ${r.amount} - 金额超限`; });
